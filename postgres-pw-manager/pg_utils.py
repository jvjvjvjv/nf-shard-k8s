...
def setup_postgres_user(pg_host, pg_port, pg_admin_user, pg_admin_password, user_name, user_dbname, user_password):
    ...
    execute_sql(user_dbname, sql.SQL("GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO {}").format(
        sql.Identifier(user_name)
    ))
    execute_sql(user_dbname, sql.SQL("GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO {}").format(
        sql.Identifier(user_name)
    ))
    ...
    execute_sql(user_dbname, sql.SQL("ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO {}").format(
        sql.Identifier(user_name)
    ))
